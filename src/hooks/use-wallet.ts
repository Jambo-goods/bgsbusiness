...
export function useWallet(triggerDeposit: boolean = false) {
  ...
  const setupRealtimeListeners = () => {
    ...
    return { walletChannel, profileChannel };
  };
  ...
  useEffect(() => {
    ...
    // If triggerDeposit is true, trigger deposit flow
    if (triggerDeposit) {
      setTimeout(() => {
        handleDeposit();
      }, 500);
    ...
    const { walletChannel, profileChannel } = setupRealtimeListeners();
