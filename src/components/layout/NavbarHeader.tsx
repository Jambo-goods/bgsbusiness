        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            <div className="relative dashboard-menu-dropdown">
              ...
              {isDashboardMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white shadow-lg rounded-lg z-50 border border-gray-100 py-2">
                  <div className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase">Menu rapide</div>
                  <Link to="/dashboard" className={`flex items-center px-4 py-2 text-sm ${isActive('/dashboard') ? 'bg-gray-100 text-bgs-blue' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <LayoutDashboard className="h-4 w-4 mr-2 text-bgs-blue" />
                    Tableau de bord
                  </Link>
                  <Link to="/dashboard?tab=wallet" className={`flex items-center px-4 py-2 text-sm ${isActive('tab=wallet') ? 'bg-gray-100 text-bgs-blue' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <Wallet className="h-4 w-4 mr-2 text-bgs-blue" />
                    Solde disponible
                  </Link>
                  <Link to="/dashboard?tab=capital" className={`flex items-center px-4 py-2 text-sm ${isActive('tab=capital') ? 'bg-gray-100 text-bgs-blue' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <TrendingUp className="h-4 w-4 mr-2 text-bgs-blue" />
                    Capital investi
                  </Link>
                  <Link to="/dashboard?tab=yield" className={`flex items-center px-4 py-2 text-sm ${isActive('tab=yield') ? 'bg-gray-100 text-bgs-blue' : 'text-gray-700 hover:bg-gray-100'}`}>
                    <BarChart3 className="h-4 w-4 mr-2 text-bgs-blue" />
                    Rendement mensuel
                  </Link>
                  <Link to="/dashboard?tab=investments" className={`flex items-center px-4 py-2 text-sm ${isActive('tab=investments') ? 'bg-gray-100 text-bgs-blue' : 'text-gray-700 hover:bg-gray-100'}`}>
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-lg z-50 border border-gray-100">
                  <div className="py-2">
                    ...
                    <hr className="my-1" />
                    <Link to="/logout" className="flex items-center px-4 py-2 text-sm text-red-500 hover:bg-gray-100">
