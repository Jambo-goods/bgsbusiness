...
export default function DashboardMenuDropdown({ isOpen, isActive }: DashboardMenuDropdownProps) {
  ...
  return (
    <div className="absolute right-0 mt-2 w-64 bg-white shadow-lg rounded-lg z-50 border border-gray-100 py-2 animate-fade-in">
      ...
      <Link to="/dashboard?tab=parrainage" className={`flex items-center px-4 py-2.5 text-sm ${isActive('tab=parrainage') ? 'bg-gray-50 text-bgs-blue font-medium' : 'text-gray-700 hover:bg-gray-50'}`}>
        <UserPlus className="h-4 w-4 mr-3 text-bgs-blue" />
        Parrainage
      </Link>
    </div>
  );
