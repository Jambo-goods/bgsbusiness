...
export default function ProjectInvestmentSimulator({
  project
}: ProjectInvestmentSimulatorProps) {
  const navigate = useNavigate();
  const [investmentAmount, setInvestmentAmount] = useState<number>(project.minInvestment);
  const [duration, setDuration] = useState<number>(project.possibleDurations ? project.possibleDurations[0] : 12);
  ...
  const handleInvest = async () => {
    ...
    try {
      ...
      const {
        ...
        user_id: user.id,
        increment_amount: -investmentAmount
      });
      if (balanceError) {
  ...
  return <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 animate-fade-up">
      ...
      <div className="mb-6">
        ...
        <Slider value={[investmentAmount]} min={project.minInvestment} max={Math.min(project.price, 20000)} step={100} onValueChange={value => setInvestmentAmount(value[0])} className="mb-2" />
      ...
      <div className="mb-6">
        ...
        {project.possibleDurations && <div className="flex justify-between gap-2 mb-2">
            {project.possibleDurations.map(months => <button key={months} onClick={() => setDuration(months)} className={`flex-1 py-2 px-1 text-sm rounded-md transition-colors ${duration === months ? "bg-bgs-blue text-white" : "bg-gray-100 text-bgs-blue hover:bg-gray-200"}`}>
                {months} mois
              </button>)}
          </div>}
      </div>
      ...
      <div className="bg-bgs-gray-light p-4 rounded-lg mb-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-bgs-blue font-bold">{totalReturn.toLocaleString(undefined, {
              maximumFractionDigits: 2
            })} €</p>
          </div>
          <div>
            <p className="text-xs text-bgs-blue/70 mb-1">Retour mensuel estimé</p>
            <p className="text-bgs-blue font-bold">{monthlyReturn.toLocaleString(undefined, {
              maximumFractionDigits: 2
            })} €</p>
          </div>
        </div>
      ...
      {isLoggedIn && walletBalance < investmentAmount && <div className="flex items-start gap-2 text-red-600 mb-4 p-3 bg-red-50 rounded-md">
          <AlertCircle className="h-5 w-5 shrink-0 mt-0.5" />
          <p className="text-sm">
            Solde insuffisant. Vous avez {walletBalance}€ dans votre portefeuille.
            <button className="block mt-1 underline text-red-600 hover:text-red-800" onClick={() => navigate("/dashboard/wallet")}>
              Ajoutez des fonds
            </button>
          </p>
        </div>}
