...
export default function DashboardCard({
  ...
  return <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:translate-y-[-2px]">
      <div className="flex items-center justify-between mb-4">
        ...
        {changePercentage && <span className={cn("text-xs px-2.5 py-1 rounded-full font-medium flex items-center", isPositive ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600')}>
      ...
      {description}
      {changeValue && changeTimeframe && <div className="mt-2 text-xs text-bgs-gray-medium">
