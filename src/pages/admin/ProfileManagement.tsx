...
export default function ProfileManagement() {
  ...
  return <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <Input type="text" placeholder="Rechercher par nom, prénom ou email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
      </div>

      <div className="bg-white rounded-md shadow">
        {isLoading ? <div className="p-4 space-y-4">
            {[...Array(5)].map((_, index) => <div key={index} className="flex space-x-4">
            ...
            <TableBody>
              {filteredProfiles.length === 0 ? <TableRow>
                  <TableCell colSpan={9} className="text-center py-8 text-gray-500">
                    {searchTerm ? "Aucun profil trouvé pour cette recherche" : "Aucun profil disponible"}
                  </TableCell>
                </TableRow> : filteredProfiles.map(profile => <TableRow key={profile.id}>
                    ...
                    <TableCell>
                      <Badge variant={profile.online_status === 'online' ? 'default' : 'secondary'} className="flex items-center gap-1">
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="amount" type="number" value={amountToAdd} onChange={e => setAmountToAdd(e.target.value)} className="col-span-3" min="1" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsAddFundsDialogOpen(false)} disabled={isProcessing}>
              Annuler
            </Button>
            <Button onClick={handleAddFundsToAll} disabled={isProcessing} className="bg-green-600 hover:bg-green-700">
